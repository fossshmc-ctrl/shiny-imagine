...
for(const entry of entries){
  ...
  if(!fs.existsSync(source))throw new Error('V29.1 Vercel build missing '+entry);
  fs.cpSync(source,target,{recursive:true});
}
fs.writeFileSync(path.join(out,'version.json'),JSON.stringify({version:'V29.1',buildId:'v29.1-wireframe-vercel-preview-fix-20260815',builtAt:new Date().toISOString()},null,2));
...
console.log(JSON.stringify({ok:true,output:'dist',files:countFiles(out),version:'V29.1'}));
